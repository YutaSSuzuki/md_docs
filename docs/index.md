...
AI が作業する場合は、`skills/docs-as-code-developer/` のルールに従う。

初回、再開時、現在状態が曖昧な場合はこのファイルから読み始める。会話が続いていて文脈が明確な場合は、必要最小限の関連資料だけを読む。
...
AI は毎回すべての資料を読む必要はない。

| Scope | When | Read |
| --- | --- | --- |
| Full Orientation | 新しい作業、再開、広範囲変更、現在状態が不明な場合 | このファイル、運用方針、Roadmap、関連カテゴリの `index.md`、必要な Current State |
| Focused Read | 会話が続いており、対象カテゴリが明確な場合 | 関連カテゴリの `index.md`、直接関係する資料、直接関係するコード |
| Minimal Read | 小さな修正、対象ファイルが明確な場合 | 対象ファイル、必要な直接リンク先のみ |
